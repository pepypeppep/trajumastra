...
class PermohonanRekomendasiBbmService
{
    /* Get alls */
    public function getAll()
    {
        ...
        return DataTables::eloquent($data)
            ...
            ->addColumn('aksi', function ($row) {
                ...
                $btnDelete = '';
                // Btn Edit
                if (auth()->user()->can('kelola-permohonan-rekomendasi-bbm.update')) {
                    $btnEdit = '<button type="button" title="Ubah data harga ikan" id="btn-modal-edit"
                        data-id="' . $row->id . '"  data-url-action="' . route('kelola.permohonan-rekomendasi-bbm.update', $row->harga_id) . '" data-url-get="' . route('kelola.permohonan-rekomendasi-bbm.edit', $row->harga_id) . '"
                ...
                // Btn Delete
                if (auth()->user()->can('kelola-permohonan-rekomendasi-bbm.delete')) {
                    $btnDelete = '<button type="button" title="Hapus data harga ikan" id="btn-delete" onclick="confirmDelete(this)"
                        data-id="' . $row->id . '"  data-url-action="' . route('kelola.permohonan-rekomendasi-bbm.destroy', $row->harga_id) . '"

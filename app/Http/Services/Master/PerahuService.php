...
class PerahuService
{
    ...
    {
        $data = MasterPerahu::orderBy('name');
        ...
        return DataTables::eloquent($data)
            ...
            ->addColumn('aksi', function ($row) {
                ...
                $btnDelete = '';
                // Btn Edit
                if (auth()->user()->can('perahu.update')) {
                    $btnEdit = '<button href="javascript:void(0);" title="Edit data perahu" id="btn-modal-edit-user"
                        data-id="' . $row->id . '"  data-url-action="' . route('master.perahu.update', $row->id) . '" data-url-get="' . route('master.perahu.edit', $row->id) . '"
                ...
                // Btn Delete
                if (auth()->user()->can('perahu.delete')) {
                    $btnDelete = '<button href="javascript:void(0);" title="Hapus data perahu" id="btn-delete-user" onclick="confirmDelete(this)"
                        data-id="' . $row->id . '"  data-url-action="' . route('master.perahu.destroy', $row->id) . '"
    ...
    {
        $data = MasterPerahu::findOrFail($id);

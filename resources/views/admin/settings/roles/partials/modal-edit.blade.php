...
<div id="modal-edit-role_{{ $role->id }}" modal-center
    ...
    <div class="w-screen md:w-[40rem] bg-white shadow rounded-md dark:bg-zink-600 flex flex-col h-full">
        <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-zink-500">
            <h5 class="text-16" id="modal-title">Edit Peran</h5>
            <button data-modal-close="modal-edit-role_{{ $role->id }}"
                class="transition-all duration-200 ease-linear text-slate-500 hover:text-red-500 dark:text-zink-200 dark:hover:text-red-500"><i
                    data-lucide="x" class="size-5"></i></button>
        </div>
        <form action="{{ route('settings.roles.update', $role->id) }}" method="POST" id="form-edit">
            @csrf
            @method('PUT')
            {{-- Start Modal Body --}}
            ...
            <div class="flex items-center justify-between p-4 mt-auto border-t border-slate-200 dark:border-zink-500">
                <button type="submit"
                    ...
                    <i class="ri-save-line"></i> Simpan perubahan data peran
                </button>
            </div>
            {{-- End Modal Footer --}}
        </form>

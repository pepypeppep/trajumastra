...
<div id="modal-edit" modal-center
    ...
    <div class="w-screen md:w-[40rem] bg-white shadow rounded-md dark:bg-zink-600 flex flex-col h-full">
        <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-zink-500">
            <h5 class="text-16" id="modal-title">Ubah Jenis Usaha</h5>
            ...
            <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                ...
                <div class="mb-1">
                    ...
                    <input type="text" id="name" name="name"
                        ...
                        placeholder="Masukkan nama jenis usaha-sarana" required>
                </div>
                {{-- Kelompok --}}
                <div class="mt-3">
                    <label for="" class="inline-block mb-2 text-base font-medium">Kelompok <strong
                            class="text-red-500">*</strong></label>
                    <select
                        ...
                        name="category_group_name" id="category_group_name">
                        <option value=""></option>
        ...
        $(document).on('click', '#btn-modal-edit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    // Modal title
                    $('#modal-title').text('Ubah Data Jenis Usaha - ' + response.name);
                    // Set form action
                    $('#form-edit').attr('action', urlFormAction);
                    // Set value to form inputs
                    $('#form-edit').find('#name').val(response.name);
                    $('#form-edit').find('#category_group_name').val(response.category_group_name)
                        .trigger('change');
                },
                error: function(xhr) {
                    Swal.fire({

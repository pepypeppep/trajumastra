...
<div id="modal-edit" modal-center
    ...
    <div class="w-screen md:w-[40rem] bg-white shadow rounded-md dark:bg-zink-600 flex flex-col h-full">
        <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-zink-500">
            <h5 class="text-16" id="modal-title">Ubah Jenis KlasifikasiUsaha</h5>
            ...
            <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                ...
                <div class="mb-1">
                    ...
                    <input type="text" id="name" name="name"
                        ...
                        placeholder="Masukkan nama jenis klasifikasi usaha" required>
                </div>
            </div>
            {{-- End Modal Body --}}
            {{-- Start Modal Footer --}}
    ...
    <script>
        $(document).ready(function() {
            $('#role-select-edit').select2({
                width: '100%',
                placeholder: "Pilih peran jenis klasifikasi usaha, bisa lebih dari satu peran",
                allowClear: true
            });
        });
    </script>
        ...
        $(document).on('click', '#btn-modal-edit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    // Modal title
                    $('#modal-title').text('Ubah Data Jenis KlasifikasiUsaha - ' + response.name);
                    // Set form action

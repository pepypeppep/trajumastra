...
<div id="modal-edit" modal-center
    ...
    <div class="w-screen md:w-[40rem] bg-white shadow rounded-md dark:bg-zink-600 flex flex-col h-full">
        <div class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-zink-500">
            <h5 class="text-16" id="modal-title">Ubah Bentuk Usaha</h5>
        ...
        <form action="#" method="POST" id="form-edit">
            ...
            <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                {{-- Nama --}}
                <div class="mb-1">
                    <label for="" class="inline-block mb-2 text-base font-medium">Nama Bentuk Usaha<strong
                            class="text-red-500">*</strong></label>
                    <input type="text" id="name" name="name"
                        class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                        placeholder="Masukkan nama bentuk usaha" required>
                </div>
            </div>
            {{-- End Modal Body --}}
            {{-- Start Modal Footer --}}
        ...
        $(document).on('click', '#btn-modal-edit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    // Modal title
                    $('#modal-title').text('Ubah Data Bentuk Usaha - ' + response.name);
                    // Set form action
                ...
                error: function(xhr) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Gagal memuat bentuk usaha.',
                    });
                }
            });
        });

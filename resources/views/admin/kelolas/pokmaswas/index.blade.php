...
@section('title', 'Kelola pokmaswas')

@section('breadcrumb')
    {{ Breadcrumbs::render('kelola.pokmaswas') }}
@endsection
...
@section('content-admin')
    <div class="card">
        <div class="card-body">
            <div class="flex justify-between items-center mb-4">
                <h5 class="mb-0">Daftar Pokmaswas</h5>
                <button type="button" href="" data-modal-target="modal-add"
                    ...
                    <i class="ri-user-add-line"></i> Tambah Pokmaswas
                </button>
            </div>
            <table id="data-table" class="display stripe group" style="width:100%">
                <thead>
                    <tr>
                        <th class="ltr:!text-left rtl:!text-right">Nama Pokmaswas</th>
                        <th class="ltr:!text-left rtl:!text-right">Bidang</th>
                        <th class="ltr:!text-left rtl:!text-right">Tahun</th>
                        <th class="ltr:!text-left rtl:!text-right">Kecamatan</th>
    ...
    {{-- Modal Add --}}
    @include('admin.kelolas.pokmaswas.partials.modal-add')
    {{-- Modal Edit --}}
    @include('admin.kelolas.pokmaswas.partials.modal-edit')
    {{-- Form Delete --}}
...
@push('scripts')
    ...
    <script>
        // Init global Select2
        function initSelect2Kecamatan(context) {
            $(context).find('[name="kecamatan_id"]').select2({
        ...
        function initSelect2Bidang(context) {
            $(context).find('[name="bidang_id[]"]').select2({
                dropdownParent: $(context),
                width: '100%',
        ...
        $(document).on('click', '[data-modal-target="modal-add"]', function() {
            initSelect2Kecamatan('#modal-add');
            initSelect2Bidang('#modal-add');
        });

        // Modal EDIT
        $(document).on('click', '[data-modal-target="modal-edit"]', function() {
            initSelect2Kecamatan('#modal-edit');
            initSelect2Bidang('#modal-edit');
        });
    </script>
    {{-- End Select 2 --}}
    ...
    <script>
        ...
        function loadTable(filter) {
            var tbl = $('#data-table').DataTable({
                ...
                ajax: {
                    url: "{{ route('kelola.pokmaswas.index') }}",
                    type: 'GET',
                },
                columns: [
                    {
